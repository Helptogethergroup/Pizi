...
<div class="flex items-center justify-between mb-6">
    <div>
        ...
        <p class="text-ink-900/60 mt-1">All blog posts on the platform</p>
    </div>
    <a href="{{ route('admin.blogs.create') }}" class="px-5 py-3 bg-coral-500 text-white rounded-xl font-bold hover:bg-coral-600">+ New Blog</a>
</div>
...
<div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
    ...
    <div class="p-4 rounded-2xl bg-amber-100 text-amber-900">
        <div class="text-xs uppercase opacity-80">Drafts</div>
...
<form class="flex flex-wrap gap-2 mb-4 bg-white p-3 rounded-xl border border-ink-900/10">
    <input name="search" value="{{ request('search') }}" placeholder="Search blog title…" class="flex-1 min-w-[200px] px-3 py-2 rounded-lg border border-ink-900/15">
...
<table class="w-full text-sm">
    ...
    @forelse($blogs as $b)
        <tr class="border-t border-ink-900/5 hover:bg-cream/40">
            <td class="px-4 py-3">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <div class="text-xs text-ink-900/50 line-clamp-1">{{ $b->excerpt }}</div>
                    </div>
                </div>
            </td>
            <td>
                <div class="font-semibold">{{ $b->author?->name ?? '—' }}</div>
                <div class="text-xs text-ink-900/50 capitalize">{{ $b->author?->role }}</div>
            </td>
            ...
            <td class="text-xs text-ink-900/60">
                {{ $b->published_at?->format('d M Y') ?? $b->created_at->format('d M Y') }}
            </td>
            <td class="px-4 py-3 text-right">
                <div class="flex flex-wrap gap-1 justify-end">
                    @if($b->is_published)
                        <a href="{{ route('blog.show', $b->slug) }}" target="_blank" class="text-xs px-2 py-1 rounded-lg border border-ink-900/15 hover:bg-cream">👁 View</a>
                    @endif
                    <a href="{{ route('admin.blogs.edit', $b) }}" class="text-xs px-2 py-1 rounded-lg bg-blue-500 text-white hover:bg-blue-600">✏️ Edit</a>
                    <form method="POST" action="{{ route('admin.blogs.toggle', $b) }}" class="inline">@csrf @method('PATCH')
                        <button class="text-xs px-2 py-1 rounded-lg {{ $b->is_published ? 'bg-amber-500' : 'bg-emerald-500' }} text-white">
                            {{ $b->is_published ? '⏸ Unpublish' : '🚀 Publish' }}
                        </button>
                    </form>
                    <form method="POST" action="{{ route('admin.blogs.destroy', $b) }}" class="inline" onsubmit="return confirm('Delete this blog permanently?');">@csrf @method('DELETE')
    ...
    @empty
        <tr><td colspan="6" class="px-4 py-12 text-center text-ink-900/50">No blogs yet. <a href="{{ route('admin.blogs.create') }}" class="text-coral-600 font-semibold">+ Create the first one</a></td></tr>
    @endforelse
    </tbody>
</table>
</div>
